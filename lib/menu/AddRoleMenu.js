...
const ViewRoleByNameQuery = require('../query/ViewRoleByNameQuery');
...
class AddRoleMenu extends Menu {
        ...
        super(db, [
            {
                ...
                validate: AddRoleMenu.#validateRoleTitle(db)
            },
            {
                name: "salary",
    ...
    /* Validates the role title field.*/
    static #validateRoleTitle(db) {
        ...
        return (answer) => {
            ...
            return new Promise((resolve, reject) => {
                ...
                if (!answer.trim().length) {
                    resolve("ERROR: Expected title to be a non-empty string");
                } else {
                    new ViewRoleByNameQuery(db, answer).query()
                        .then(([rows, fields]) => {
                            if (rows.length) {
                                /* If the role already exists, the input is invalid. */
                                resolve("Error: A role already exists with that title");
                            } else {
